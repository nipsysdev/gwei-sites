 ...
 *
 * Lowercases and trims the label, then appends the `.gwei` suffix. This
 * prevents case-sensitivity issues where `Xav.gwei.site` would resolve
 * differently than `xav.gwei.site`.
 *
 * @param sub  The subdomain label extracted from the Host header.
 * @returns    Normalized full name, e.g. `"xav.gwei"`.
 */
